...
elseif(isset($_GET['category']) && preg_match("/^[a-zA-Z]*$/", $_GET['category']))
{
	$id = $_GET['category'];
	$selector = "category";
}
...
$query = "SELECT title, full_desc FROM pages WHERE $selector='$id'";
...
$smarty->assign('title', $results[0]['title']);
$smarty->assign('full_desc', $results[0]['full_desc']);
